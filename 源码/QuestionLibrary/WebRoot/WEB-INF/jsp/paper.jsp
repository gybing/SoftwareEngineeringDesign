...
<body style="text-align: center;">
    ...
    <table frame="border">
      <form method="post" action="${pageContext.request.contextPath }/CheckServlet"> 
        <tr>
            <td><font size="18">${paper.papername }</font></td>
        </tr>
        <tr>
	...
	<c:if test="${!empty(xuanze) }">
		...
		<c:forEach items="${xuanze }" var="xz">
		    ...
		    <tr>
                <td>
                    <input type="radio" name="${xz.quesid }" value="A">${xz.other1 }&nbsp;&nbsp; 
	...
	<c:if test="${!empty(tiankong) }">
		...
		<c:forEach items="${tiankong }" var="tk">
		    ...
		    <tr>
                <td>
                   <input type="text" name="${tk.quesid }">
	            </td>
            </tr>
		</c:forEach>
	</c:if>
	<c:if test="${!empty(jeishi) }">
		...
		<c:forEach items="${jeishi }" var="js">
		    ...
		    <tr>
                <td>
                   <textarea rows="5" cols="50" name="${js.quesid }"></textarea>
	            </td>
            </tr>
		</c:forEach>
	</c:if>
	<c:if test="${!empty(jianda) }">
		...
		<c:forEach items="${jianda }" var="jd">
		    ...
		    <tr>
                <td>
                   <textarea rows="5" cols="50" name="${jd.quesid }"></textarea>
	            </td>
            </tr>
		</c:forEach>
	</c:if>
	<c:if test="${!empty(yingyong) }">
		...
		<c:forEach items="${yingyong }" var="yy">
		    ...
		    <tr>
                <td>
                   <textarea rows="5" cols="50" name="${yy.quesid }"></textarea>
	            </td>
            </tr>
		</c:forEach>
	</c:if>
